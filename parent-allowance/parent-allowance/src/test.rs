...
use soroban_sdk::{
    ...
    vec, AccountId, BytesN, Env, IntoVal, Vec
};
...
fn test_invalid_contract_initialize_panics_when_contract_is_initialized() {
    ...
    client.initialize(&admin, &contract_id, &step_period, &start_period, &end_period);
}
...
fn test_valid_initialized_periods() {
    ...
    assert_eq!(1669726146, client.getStartP());

    assert_eq!(86400, client.getStepP());

    assert_eq!(0, client.getEndP());
...
fn test_valid_children_registration() {
    ...
    client.setAllow(&child_a_account, &child_a_allowance);

    assert_eq!(child_a_allowance, client.getAllow(&child_a_account));
    ...
    client.setAllow(&child_b_account, &child_b_allowance);

    assert_eq!(child_b_allowance, client.getAllow(&child_b_account));
}
...
fn test_valid_sequence_withdraw() {
    ...
    client.initialize(&admin, &payment_tkn_id, &step_period, &start_period, &end_period);
    ...
    assert_eq!(50, client.getWthdrwn(&child_a_account), "child A withdraws 50, must have 50 withdrawn ");
    assert_eq!(50, client.getAlwAv(&child_a_account), "child A withdraws 50, must have 50 left");
    ...
    assert_eq!(70, client.getWthdrwn(&child_b_account), "child B withdraws 70, must have 70 withdrawn");
    assert_eq!(230, client.getAlwAv(&child_b_account), "child B withdraws 70, must have 230 left");
